...
@Component({
  selector: 'app-disbursementapp',
  templateUrl: './disbursementapp.component.html',
  styleUrls: ['./disbursementapp.component.scss']
})
export class DisbursementappComponent implements OnInit {
  datalist: any;
  loading: any;
  rownum: any;
  file: any;
  ...
  dataName: any;
  dataNameb: any;
  url = "/acc3d/appmoney/annal/disbursementapp.php";
 ...
 var varN1 = {
  ...
  editdata(id: any, cizb: any) {
    this.setshowbti();
    this.fetchdatareport();
    this.dataAdd.FNANNALSMAP_CODE = id;
     this.dataAdd.CITIZEN_IDA = cizb;
  ...
  // ฟังก์ขันสำหรับการนำข้อมูลมาแสดงเพื่อแก้ไข
  editdatapp(id: any, ciz: any, cizb: any) {
    this.setshowbti();
    this.fetchdatareport();
    this.dataAdd.FNANNALSMAP_CODE = id;
  ...
  insertdata() {
    if (this.dataAdd.CITIZEN_IDB  == '') {
        this.toastr.warning("แจ้งเตือน:กรุณาคนเสนอเขียนเช็ค");
  ...
  updatedata() {
    ...
    this.apiService
      ...
      .subscribe((data: any) => {
        ...
        if (data.status == 1) {
         /* this.Uploadfiles.uploadcheck(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, this.dataAdd.FNANNALS_CODE, this.dataAdd.citizen)
            .subscribe((event: any) => {
              // 
              if (event.type == 4) {
              }
