      ...
      .subscribe((data: any) => {
        ...
        this.apiService
          ...
          .subscribe((datac: any) => {
            ...
            //this.dataAdd.CAMPUS_CODE = datac[0].CAMPUS_CODE;
          });
        //รายการปี
        var Table = {
          "opt": "viewyear"
  ...
  async openPdfAnnotator1(p: any) {
    ...
    if (data && data.saved && data.blob) {
      ...
      const file = new File([data.blob], 'signed_document.pdf', { type: 'application/pdf' });
          ...
          this.Uploadfiles.uploadcheck(file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, p.FNANNALSMAP_CODE1, user.citizen, '81')
            .subscribe((event: any) => {
              if (event.type == 4) {
                              // หลังจากอัปโหลดสำเร็จ ให้บันทึกสถานะ
                              this.dataAdd.opt = "sendfile";
                              this.dataAdd.FNANNALSMAP_CODE=p.FNANNALSMAP_CODE1;
                              this.apiService.getdata(this.dataAdd, this.url)
                                .pipe(first())
                                .subscribe((data: any) => {
                                  if (data.status == 1) {
