...
@Component({
  selector: 'app-annalsend',
  templateUrl: './annalsend.component.html',
  styleUrls: ['./annalsend.component.scss']
})
export class AnnalsendComponent implements OnInit {
title = 'angular-app';
  ...
  dataNameb: any;
  dataSeq: any;
  url = "/acc3d/appmoney/annal/annalsend.php";
 ...
 fetchdata() {
  ...
  editdata(id: any, fac: any,link: any) {
    ...
    this.apiService
      ...
      .pipe(first())
      .subscribe((data: any) => {
        this.dataAdd.FNANNALSMAPR_CODE = data.data[0].FNANNALSMAPS_CODE;
  ...
  insertdata() {
    ...
    if (this.dataAdd.EBOOKREQ_FILE == '' || num==0) {
     if (this.dataAdd.EBOOKREQ_FILE == '') {
      ...
      this.apiService
        ...
        .subscribe((data: any) => {
          ...
          if (data.status == 1) {
            this.Uploadfiles.uploadcontract(this.file, data.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, data.FNANNALSMAPR_CODE, this.dataAdd.citizen, '83')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                  this.fetchdatalist();
    ...
    if (num == 0) {
      ...
      this.apiService
        ...
        .subscribe((data: any) => {
          //console.log(data.status);       
          if (data.status == 1) {
            this.Uploadfiles.uploadcontract(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, this.dataAdd.FNANNALSMAPR_CODE, this.dataAdd.citizen, '83')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                  this.fetchdatalist();
