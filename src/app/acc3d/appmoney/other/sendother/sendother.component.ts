...
@Component({
  selector: 'app-sendother',
  templateUrl: './sendother.component.html',
  styleUrls: ['./sendother.component.scss']
})
export class SendotherComponent implements OnInit {
  datalist: any;
  loading: any;
  rownum: any;
  file: any;
  ...
  dataName: any;
  dataNameb: any;
  url = "/acc3d/appmoney/other/sendother.php";
    ...
    if (this.dataAdd.EBOOKREQ_FILE == ''  || this.dataAdd.CHIEF_CODE == '' || this.dataAdd.DEPARTMENT_CODE == '') {
      ...
      this.apiService
        ...
        .subscribe((data: any) => {
          ...
          if (data.status == 1) {
            this.Uploadfiles.uploadcontract(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, this.dataAdd.FNANNALSMAP_CODE, this.dataAdd.citizen, '98')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                }
