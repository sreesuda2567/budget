...
@Component({
  selector: 'app-checkother',
  templateUrl: './checkother.component.html',
  styleUrls: ['./checkother.component.scss']
})
export class CheckotherComponent implements OnInit {
  datalist: any;
  loading: any;
  rownum: any;
  file: any;
  ...
  dataName: any;
  dataNameb: any;
  url = "/acc3d/appmoney/other/checkother.php";
...
fetchdata() {
  ...
  editdatapp(id: any, id2: any, link: any, ciz: any, edoc: any, chief: any, mon: any) {
    this.setshowbti();
    this.fetchdatareport();
    this.onChangeedoc();
    this.onChangechief();
    ...
    if (this.dataAdd.EBOOKREQ_FILE == ''  || this.dataAdd.CHIEF_CODE == '' || this.dataAdd.DEPARTMENT_CODE == '') {
      ...
      this.apiService
        ...
        .subscribe((data: any) => {
          ...
          if (data.status == 1) {
            this.Uploadfiles.uploadcontract(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE,this.dataAdd.FNANNALSMAP_CODE, this.dataAdd.citizen, '105')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                }
