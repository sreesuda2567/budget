...
@Component({
  selector: 'app-load3dm',
  templateUrl: './load3dm.component.html',
  styleUrls: ['./load3dm.component.scss']
})
export class Load3dmComponent implements OnInit {
 datalist: any;
  loading: any;
  rownum: any;
  file: any;
  ...
  dataName: any;
  dataNameb: any;
  url = "/acc3d/appmoney/other/load3dm.php";
...
fetchdata() {
  ...
  setshowbti() {
    ...
    this.dataAdd.FNANNALS_MONEYC = 0;
  ...
  editdata(id: any,id2: any, no: any, mon: any,link:any) {
    this.setshowbti();
  ...
  editdatapp(id: any, id2: any, link: any, ciz: any, edoc: any, chief: any, deka: any) {
    this.setshowbti();
    this.fetchdatareport();
    this.onChangeedoc();
    this.onChangechief();
    ...
    if (this.dataAdd.EBOOKREQ_FILE == ''  || this.dataAdd.CHIEF_CODE == '' || this.dataAdd.DEPARTMENT_CODE == '') {
      ...
      this.apiService
        ...
        .subscribe((data: any) => {
          ...
          if (data.status == 1) {
            this.Uploadfiles.uploadcontract(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, data.id, this.dataAdd.citizen, '27')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                }
