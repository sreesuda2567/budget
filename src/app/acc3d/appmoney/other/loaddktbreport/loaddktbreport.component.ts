...
@Component({
  selector: 'app-loaddktbreport',
  templateUrl: './loaddktbreport.component.html',
  styleUrls: ['./loaddktbreport.component.scss']
})
export class LoaddktbreportComponent implements OnInit {
  datalist: any;
  loading: any;
  rownum: any;
  file: any;
  ...
  dataName: any;
  dataNameb: any;
  url = "/acc3d/appmoney/other/loaddktbreport.php";
...
fetchdata() {
  ...
  // ฟังก์ขันสำหรับการนำข้อมูลมาแสดงเพื่อแก้ไข
  editdatapp(id: any, ciz1: any, link: any) {
    this.setshowbti();
    this.fetchdatareport();
    this.dataAdd.FNANNALSMAP_CODE = id;
    this.dataAdd.CITIZEN_IDP1 = ciz1;
    this.dataAdd.EBOOKREQ_LINK = link
    this.rowpbi = '';
    this.rowpbu = 1;
  }
  ...
  insertdata() {
  // console.log(this.dataAdd.FNANNALSMAP_CODE);
    if (this.dataAdd.CITIZEN_IDP1 == '') {
      this.toastr.warning("แจ้งเตือน:กรุณาการเงินผู้รับผิดชอบ");

    } else {
      this.Uploadfiles.uploadcontract(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, this.dataAdd.FNANNALSMAP_CODE, this.dataAdd.citizen, '84')
        .subscribe((event: any) => {
          if (event.type == 4) {
