...
@Component({
  selector: 'app-paymentdate',
  templateUrl: './paymentdate.component.html',
  styleUrls: ['./paymentdate.component.scss']
})
export class PaymentdateComponent implements OnInit {
  ...
  dataName: any;
  dataNameb: any;
  url = "/acc3d/appmoney/other/paymentdate.php";
...
fetchdata() {
  ...
  insertdata() {
    if (this.dataAdd.CITIZEN_IDK1 == '') {
      this.toastr.warning("แจ้งเตือน:กรุณาเลือกผู้ขออนุมัติ");
      ...
      this.apiService
        ...
        .subscribe((data: any) => {
          ...
          if (data.status == 1) {
            ...
            this.Uploadfiles.uploadcontract(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, this.dataAdd.FNANNALSMAP_CODE, this.dataAdd.citizen, '85')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                  this.fetchdatalist();
