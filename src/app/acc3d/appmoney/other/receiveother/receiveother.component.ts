...
@Component({
  selector: 'app-receiveother',
  templateUrl: './receiveother.component.html',
  styleUrls: ['./receiveother.component.scss']
})
...
datalist: any;
  ...
  dataName: any;
  dataNameb: any;
  url = "/acc3d/appmoney/other/receiveother.php";
...
fetchdata() {
      ...
      .subscribe((data: any) => {
        if (data && data.length > 0) {
          ...
          this.apiService
            ...
            .subscribe((data2: any) => {
              this.dataName = data2;
            //  this.dataAdd.CITIZEN_IDK1 = data2[0]?.CITIZEN_ID;
             // this.dataAdd.CITIZEN_IDK2 = data2[1]?.CITIZEN_ID;
            });
        }
      });
    var varP = {
  ...
  insertdata() {
   if (this.dataAdd.EBOOKREQ_FILE == '' || this.dataAdd.CITIZEN_IDK2 == '') {
      if (this.dataAdd.EBOOKREQ_FILE == '' ) {
      this.toastr.warning("แจ้งเตือน:กรุณาแนบไฟล์");
      }
      if (this.dataAdd.CITIZEN_IDK2 == '' ) {
      this.toastr.warning("แจ้งเตือน:กรุณาเลือกผู้ส่งประจำวัน");
      }
      ...
      this.apiService
        ...
        .subscribe((data: any) => {
          ...
          if (data.status == 1) {
             this.Uploadfiles.uploadcontract(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, this.dataAdd.FNANNALSMAP_CODE, this.dataAdd.citizen, '102')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                  this.fetchdatalist();
      ...
      this.apiService
        ...
        .subscribe((data: any) => {
          ...
          if (data.status == 1) {
            ...
            this.Uploadfiles.uploadcontract(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, this.dataAdd.FNANNALSMAP_CODE, this.dataAdd.citizen, '107')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                  this.fetchdatalist();
