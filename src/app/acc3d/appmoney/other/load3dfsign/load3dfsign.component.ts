...
@Component({
  selector: 'app-load3dfsign',
  templateUrl: './load3dfsign.component.html',
  styleUrls: ['./load3dfsign.component.scss']
})
export class Load3dfsignComponent implements OnInit {
  datalist: any;
  loading: any;
  rownum: any;
  file: any;
  ...
  dataName: any;
  dataNameb: any;
  url = "/acc3d/appmoney/other/load3dfsign.php";
    ...
    if (this.dataAdd.EBOOKREQ_FILE == '') {
      ...
      this.apiService
        ...
        .subscribe((data: any) => {
          ...
          if (data.status == 1) {
            this.Uploadfiles.uploadcontract(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, this.dataAdd.FNANNALSMAP_CODE, this.dataAdd.citizen,'33')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                  this.fetchdatalist('0');
