...
@Component({
  selector: 'app-formatr',
  templateUrl: './formatr.component.html',
  styleUrls: ['./formatr.component.scss']
})
...
editdata(id: any) {
  ...
  this.apiService
    ...
    .subscribe((data: any) => {
      ...
      if(data[0].BDFORMAT_TDATEE !=null){
        this.dataAdd.BDOPERATIONTD_TDATE[6] = new Date(data[0].BDFORMAT_TDATEE);
      }
      ...
      if(data[0].BDFORMAT_TDATER !=null){
        this.dataAdd.BDOPERATIONTD_TDATE[7] = new Date(data[0].BDFORMAT_TDATER);
      }
      if(data[0].BDFORMAT_SDATES !=null){  
      ...
      if(data[0].BDFORMAT_TDATEMT !=null){
        this.dataAdd.BDOPERATIONTD_TDATE[8] = new Date(data[0].BDFORMAT_TDATEMT);
      }
    });
  this.rowpbi = null;
  this.rowpbu = true;
...
updatedata(){ 
  ...
  this.toastr.warning("แจ้งเตือน:กรุณาเลือกวันที่ประชุมสรุปความต้องการให้ครบถ้วน");
    ...
    this.toastr.warning("แจ้งเตือน:กรุณาเลือกวันที่ประชุมคณะกรรมการแบบรูปรายการให้ครบถ้วน");
     ...
     if(this.dataAdd.BDOPERATIONTD_TDATE[6] !=''){
      this.dataAdd.BDOPERATIONTD_TDATE[6] = this.datenow(this.dataAdd.BDOPERATIONTD_TDATE[6]);
     }

     if(this.dataAdd.BDOPERATIONTD_TDATE[7] !=''){
      this.dataAdd.BDOPERATIONTD_TDATE[7] = this.datenow(this.dataAdd.BDOPERATIONTD_TDATE[7]);
     }

     if(this.dataAdd.BDOPERATIONTD_TDATE[8] !=''){
      this.dataAdd.BDOPERATIONTD_TDATE[8] = this.datenow(this.dataAdd.BDOPERATIONTD_TDATE[8]);
     }
