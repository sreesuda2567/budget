...
@Component({
  selector: 'app-reportyexpenses',
  templateUrl: './reportyexpenses.component.html',
  styleUrls: ['./reportyexpenses.component.scss']
})
export class ReportyexpensesComponent implements OnInit {
 title = 'angular-app';
  ...
  locale = 'th-be';
  locales = listLocales();
  url = "/acc3d/budget/report/report_yexpenses.php";
  ...
  fetchdatalist() {
    ...
    this.apiService
      ...
      .subscribe((data: any) => {
        ...
        this.dataPlmoney = data.data2;
        this.dataAdd.SUMT= data.sumt;
        this.loading = null;
        this.rownum = 1;
        }else{
          this.rownum = null;
...
exportexcel(): void {
    ...
    const numberCols = [2,3,4,5,6,7,8,9,10];

    for (let R = range.s.r; R <= range.e.r; ++R) {
      const isBoldRow = (R === 0|| R === 1 );
