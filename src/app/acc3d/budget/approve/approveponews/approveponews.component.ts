...
@Component({
  selector: 'app-approveponews',
  templateUrl: './approveponews.component.html',
  styleUrls: ['./approveponews.component.scss']
})
export class ApproveponewsComponent implements OnInit {
  title = 'angular-app';
  ...
  dataEpl: any;
  searchTerm: any;
  url = "/acc3d/budget/approve/approve_ponews.php";
  url1 = "/acc3d/budget/userpermission.php";
  dataAdd: any = {};
  dataFAdd: any = {
    FRACC1: [], FNEXACCCODE: [], FRACCCODE: [], FNEXACCMONEY: [], FNEXACCRDATE: [], FNEXACCDEKA: [], FNEXACCCODEC: []
    , FRACCCODEC: [], FNEXACCMONEYC: [], FNEXACC_DATE: [], FNEXACC_DATE1: [], FNEXACC_TYPE: [], FNEXACC_NUMBER: [], FNEXPENSES_DELIVERRY: [], FNEXPENSES_ORDER: []
    , FNEXACC_ADATE: [], FNEXACC_ADATE1: []
  };
  page = 1;
  count = 0;
  tableSize = 20;
  ...
  selectEvenf(item: any) {
    this.dataFAdd.FNCREDITOR_CODE = item.id;
    console.log(item);
  }
  onChangeSearchf(val: string) {
    var varP = {
      "opt": "viewFNCREDITOR",
      "search": val
    }
  ...
  cleardata() {
    for (let i = 0; i < this.dataFAdd.FNEXACCMONEYC.length; i++) {
      ...
      this.dataFAdd.FNEXPENSES_DELIVERRY[i] = null;
      this.dataFAdd.FNEXPENSES_ORDER[i] = null;

    }
    this.dataFAdd.FNEXACC_TYPE[0] = 'WE';
    this.dataFAdd.FNCREDITOR_CODE1 = '';
    this.dataFAdd.FNCREDITOR_CODE = '';
  }
  ...
  editdata(id: any) {
    ...
    this.apiService
      ...
      .subscribe((data: any) => {
        ...
        this.dataFAdd.PLPROJECT_CODE = data[0].PLPROJECT_CODE;
        this.dataFAdd.PLMONEYPAY_CODE = data[0].PLMONEYPAY_CODE;
        this.dataFAdd.PLYEARBUDGET_CODE = data[0].PLYEARBUDGET_CODE;
        this.dataFAdd.FACULTY_CODE = data[0].FACULTY_CODE;
        this.dataFAdd.PLINCOME_CODE = data[0].PLINCOME_CODE;
        this.dataFAdd.CRPART_ID = data[0].CRPART_ID;
    ...
    this.apiService
      ...
      .pipe(first())
      .subscribe((data: any) => {
        this.dataFAdd.FNEXPENSES_CODE = data[0].FNEXPENSES_CODE;
        this.dataFAdd.PLSUBMONEYPAY_CODE = data[0].PLSUBMONEYPAY_CODE;
        this.dataFAdd.FNEXPENSES_TITLE = data[0].FNEXPENSES_TITLE;
        this.dataFAdd.FNEXPENSES_STEP = data[0].FNEXPENSES_STEP;
        this.dataFAdd.PLGPRODUCT_CODE = data[0].PLGPRODUCT_CODE;
        this.dataFAdd.FNCREDITOR_CODE = data[0].FNCREDITOR_CODE;
        ...
        this.dataFAdd.FNEXPENSES_AMONEY = (parseFloat(data[0].FNEXPENSES_AMONEY).toFixed(2));
        this.dataFAdd.FNEXPENSES_CMDATE = new Date();
        this.dataFAdd.FNEXPENSES_DEKA = 'P';
        this.dataFAdd.FNEXACC_DATE[0] = new Date();
        this.dataFAdd.FNEXACC_ADATE[0] = new Date();
        this.dataFAdd.FNEXACCMONEYC[0] = (parseFloat(data[0].FNEXPENSES_AMONEY).toFixed(2));
  ...
  updatedata() {
    ...
    if (this.dataAdd.FNEXPENSES_DEKA == '' || this.dataAdd.FNEXPENSES_CMDATE == '' || this.dataFAdd.FRACCCODEC[0] == ''|| this.dataAdd.FNCREDITOR_CODE == '') {
      ...
      if (this.dataAdd.FNEXPENSES_CMDATE == '') {
        this.toastr.warning("แจ้งเตือน:กรุณากรอกวันที่ PO");
      ...
      if (this.dataFAdd.FNCREDITOR_CODE == '') {
        this.toastr.warning("แจ้งเตือน:กรุณาเลือกร้านค้าจากระบบ");
      }
        ...
        if (result.value) {
          ...
          for (let i = 0; i < this.dataFAdd.FNEXACC_DATE.length; i++) {
            if (this.dataFAdd.FNEXACC_DATE[i]) {
              ...
              this.dataFAdd.FNEXACC_ADATE1[i] = this.datenow(this.dataFAdd.FNEXACC_ADATE[i]);
            }
          }
