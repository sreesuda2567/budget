...
@Component({
  selector: 'app-loadw',
  templateUrl: './loadw.component.html',
...
datalist: any;
  ...
  dataName: any;
  dataNameb: any;
  url = "/acc3d/welfare/load/load3w.php";
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        try {
          ...
          // หมายเหตุ: ใช้รหัสอัปโหลด '119' (เพราะ insertdata ใช้ 118) หากต้องใช้รหัสอื่นสามารถแก้ได้เลยครับ
          this.Uploadfiles.uploadcontract(mergedFile, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, id, user.citizen, '27')
            .subscribe((event: any) => {
              if (event.type == 4) {
                Swal.close();
