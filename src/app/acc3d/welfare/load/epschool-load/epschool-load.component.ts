...
@Component({
  ...
  templateUrl: './epschool-load.component.html',
  styleUrls: ['./epschool-load.component.scss']
})
export class EpschoolLoadComponent implements OnInit {
  title = 'angular-app';
  ...
  locales = listLocales();
  rownum: any;
  url = "/acc3d/welfare/load/epschool_load.php";
  ...
  editdata(id: any) {
    ...
    this.apiService
      ...
      .subscribe((data: any) => {
       ...
       /* this.dataAdd.opt = "readone2";
        this.apiService
          .getdata(this.dataAdd, this.url)
          .pipe(first())
          .subscribe((data: any) => {
           //console.log(data.data2.length);this.dataAdd.FEREIM_CODE[i]
            for (let i = 0; i < data.data2.length; i++) {
                for (var j = 0; j < data.data.length; j++) {
                  if (data.data2[i].FEREIM_CODE == data.data[j].FEREIM_CODE) {
                    //console.log(data.data[j].FEREIM_CODE);
                    this.dataAdd.check[i] = true;
                  }else{
                    //this.dataAdd.check[i] = false;
                  } 
           ...
           console.log(this.dataAdd.FEREIM_CODE.length);
                for (let i = 0; i < 10; i++) {
                  for (var j = 0; j < data.data.length; j++) {
                    if (this.dataAdd.FEREIM_CODE[i]== data.data[j].FEREIM_CODE) {
                      console.log(data.data[j].FEREIM_CODE);
                      this.dataAdd.check[i] = true;
                    }else{
                      //this.dataAdd.check[i] = false;
