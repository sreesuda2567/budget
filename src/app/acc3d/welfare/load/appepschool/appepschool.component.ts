...
@Component({
  selector: 'app-appepschool',
  templateUrl: './appepschool.component.html',
  styleUrls: ['./appepschool.component.scss']
})
export class AppepschoolComponent implements OnInit {
title = 'angular-app';
  ...
  locales = listLocales();
  rownum: any;
  url = "/acc3d/welfare/load/appepschool.php";
    ...
    if (num == 0) {
      ...
      this.apiService
        ...
        .subscribe((data: any) => {
          ...
          if (data.status == 1) {
            this.Uploadfiles.uploadcheck(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, data.id, this.dataAdd.citizen, '116')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                  this.fetchdatalist();
    ...
    Swal.fire({
      ...
      if (result.value) {
        ...
        try {
          ...
          // หมายเหตุ: ใช้รหัสอัปโหลด '117' (เนื่องจากฟังก์ชันเพิ่มข้อมูลใช้ 116) หากต้องใช้รหัสอื่นสามารถแก้ได้เลยครับ
          this.Uploadfiles.uploadcontract(mergedFile, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE, id, user.citizen, '117')
            .subscribe((event: any) => {
              if (event.type == 4) {
                Swal.close();
