...
@Component({
  selector: 'app-regisisbuiling',
  templateUrl: './regisisbuiling.component.html',
  styleUrls: ['./regisisbuiling.component.scss']
})
export class RegisisbuilingComponent implements OnInit {
  datalist:any ;
  loading:any;
  rownum:any;
  searchTerm: any;
  ...
  dataUnit :any;
  datalistname:any;
  url = "/acc3d/investment/register/regis_building.php";
...
//รายการหน่วยนับ
 ...
 editdata(id: any){ 
  ...
  .subscribe((data: any) => {
    ...
    this.dataAdd.PRREGISBUILDING_NAME= data[0].PRREGISBUILDING_NAME;
...
insertdata(){ 
  if(this.dataAdd.SUB_DISTRICT_ID =='' || this.dataAdd.PRREGISBUILDING_NAME =='' || this.dataAdd.PLBUILDINGTYPE_CODE =='' || this.dataAdd.GCUNIT_CODE =='' ){
    if(this.dataAdd.PLBUILDINGTYPE_CODE ==''){
      this.toastr.warning("แจ้งเตือน:กรุณาเลือกประเภทสิ่งก่อสร้าง");
     }
     if(this.dataAdd.SUB_DISTRICT_ID ==''){
      this.toastr.warning("แจ้งเตือน:กรุณาเลือกพื้นที่ติดตั้ง");
     }
     if(this.dataAdd.PRREGISBUILDING_NAME ==''){
      this.toastr.warning("แจ้งเตือน:กรุณากรอกชื่อสิ่งก่อสร้าง");
     } 
