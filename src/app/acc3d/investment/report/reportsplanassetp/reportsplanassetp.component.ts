...
@Component({
  selector: 'app-reportsplanassetp',
  templateUrl: './reportsplanassetp.component.html',
  styleUrls: ['./reportsplanassetp.component.scss']
})
export class ReportsplanassetpComponent implements OnInit {
  title = 'angular-app';
  ...
  datalistdetail :any;
  dataMyear:any;
  url = "/acc3d/investment/report/reportsplanassetp.php";
...
fetchdataCam(){
  ...
  exportexcel(): void {
       ...
       // ✅ บันทึกไฟล์ Excel
       XLSX.writeFile(wb, "รายงานแผนความต้องการรายการครุภัณฑ์.xlsx");
   }
  ...
  exportexcelphp(){
    //console.log(this.dataAdd.month);
    //let citizen=this.tokenStorage.getUser().citizen;
    let url='https://budget.rmutsv.ac.th/report/exceleport_splanasset.php?CAMPUS_CODE='+btoa(this.dataAdd.CAMPUS_CODE)+'&FACULTY_CODE='+btoa(this.dataAdd.FACULTY_CODE)
    +'&SPRYEARASSET_CODE='+btoa(this.dataAdd.SPRYEARASSET_CODE)+'&EPRYEARASSET_CODE='+btoa(this.dataAdd.EPRYEARASSET_CODE)+'&PLINCOME_CODE='+btoa(this.dataAdd.PLINCOME_CODE)
    +'&CRPART_ID='+btoa(this.dataAdd.CRPART_ID)+'&PLGPRODUCT_CODE='+btoa(this.dataAdd.PLGPRODUCT_CODE)+'&PRASSET_RSTATUS='+btoa(this.dataAdd.PRASSET_RSTATUS)+'&PRIVILEGE_RSTATUS='+btoa(this.dataAdd.PRIVILEGE_RSTATUS);
