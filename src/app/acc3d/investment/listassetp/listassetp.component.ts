...
@Component({
  selector: 'app-listassetp',
  templateUrl: './listassetp.component.html',
  styleUrls: ['./listassetp.component.scss']
})
export class ListassetpComponent implements OnInit {
  name = 'Angular';
 ...
 // param: any = { MASSAGE_CITIZEN: null }
  ...
  dataYearimport:any ;
  rownumimport:any; 
  url = "/acc3d/investment/passet/listassetp.php";
...
updatespec(){   
  ...
  if(this.dataAdd.PRASSETSPEC_NAME ==''){
     if(this.dataAdd.PRASSETSPEC_NAME ==''){
      ...
      this.dataAdd.PRASSETSPEC_DETAIL = this.dataAdd.PRASSETSPEC_DETAIL.replaceAll("&nbsp;", "");
...
insertdata(){  
 ...
 //console.log(this.afyear);
    ...
    this.dataAdd.PRASSET_ANOTHER = data[0].PRASSET_SUPPORT;
...
fetchdataCam(){
  ...
  updatedata(){ 
    ...
    this.Uploadfiles.uploadasset(this.file,this.fileict,this.filerequest,this.filespec,this.dataAdd.PRYEARASSET_CODE,this.dataAdd.FACULTY_CODE,this.dataAdd.PRASSET_CODE)
    .subscribe((event: any) => {
      }
    )  ;
