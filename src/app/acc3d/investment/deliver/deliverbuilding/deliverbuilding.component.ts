...
@Component({
  selector: 'app-deliverbuilding',
  templateUrl: './deliverbuilding.component.html',
  styleUrls: ['./deliverbuilding.component.scss']
})
export class DeliverbuildingComponent implements OnInit {
  dataFac: any;
  dataIncome: any;
  dataCrpart: any;
  dataYear: any;
  ...
  url = "/acc3d/investment/deliver/deliverbuilding.php";
      ...
      .subscribe((data: any) => {
        if (data.status == 1) {
          ...
          for (let i = 0; i < this.datalistapp.length; i++) {
            ...
            this.dataAdd.PRASSET_MONEY[i] = this.datalistapp[i].PRBUILDING_MONEY*this.datalistapp[i].PRBUILDING_NUMBER;
  ...
  insertdataapp() {
    ...
    Swal.fire({
      title: 'ต้องการนำส่งข้อมูลรายการสิ่งก่อสร้าง ทั้งหมด '+this.dataAdd.check.length+' รายการ จำนวนเงิน '+this.numberWithCommas(num)+' บาท',
      icon: 'warning',
