...
@Component({
  selector: 'app-reportdaycheck',
  templateUrl: './reportdaycheck.component.html',
  styleUrls: ['./reportdaycheck.component.scss']
})
export class ReportdaycheckComponent implements OnInit {
 title = 'angular-app';
  ...
  dataNameb: any;
  datareceipt: any;
  url = "/acc3d/appmoney/report/reportdaycheck.php";
      ...
      .subscribe((data: any) => {
        if (data.status == '1') {
          ...
          for (let i = 0; i < this.datalist.length; i++) {
            ...
            this.dataAdd.FNEXACC_DETAIL[i] = 'ส่งคืนเอกสารของ '+ this.datalist[i].FSTF_FNAME+'ประจำวันที่ '+ this.datalist[i].FNANNALSMAPR_DATE1;
    ...
    if (num == 0) {
      ...
      this.apiService
        ...
        .subscribe(
          (data: any) => {
            if (data.status == '1') {
               this.Uploadfiles.uploadcontract(this.file, this.dataAdd.FACULTY_CODE, this.dataAdd.PLYEARBUDGET_CODE,  data.id, this.dataAdd.citizen, '90')
              .subscribe((event: any) => {
                // 
                if (event.type == 4) {
                }
