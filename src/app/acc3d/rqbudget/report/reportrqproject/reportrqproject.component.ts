...
@Component({
  selector: 'app-reportrqproject',
  templateUrl: './reportrqproject.component.html',
  styleUrls: ['./reportrqproject.component.scss']
})
export class ReportrqprojectComponent implements OnInit {
  title = 'angular-app';
  ...
  datadetail: any;
  datalistdetail: any;
  url = "/acc3d/rqbudget/report/reportrqproject.php";
 ...
 XLSX.utils.book_append_sheet(wb, wsdetail, 'รายการโครงการ');
  ...
  XLSX.utils.book_append_sheet(wb, ws, "สรุปรายการ");
  XLSX.utils.book_append_sheet(wb, wsDetail, "รายการโครงการ");

  // ✅ บันทึกไฟล์ Excel
  XLSX.writeFile(wb, "รายงานสรุปรายการ.xlsx");
}

// ✅ ฟังก์ชันจัดกึ่งกลาง + Merge Cell ของหัวตาราง
centerAlignHeader(ws: XLSX.WorkSheet) {
  const range = XLSX.utils.decode_range(ws["!ref"]!);

  // --- หัวข้อใหญ่ (3 บรรทัดแรก)
  ...
  ws["!merges"] = [
    // รวม 3 บรรทัดแรก (หัวข้อหลัก)
    { s: { r: 0, c: 0 }, e: { r: 0, c: range.e.c } },
    { s: { r: 1, c: 0 }, e: { r: 1, c: range.e.c } },
    { s: { r: 2, c: 0 }, e: { r: 2, c: range.e.c } },

    // ✅ หัวตารางแถว 4–5 ตามรูป
    { s: { r: 3, c: 0 }, e: { r: 4, c: 0 } }, // ลำดับ
    { s: { r: 3, c: 1 }, e: { r: 4, c: 1 } }, // ชื่อรายการ
    { s: { r: 3, c: 2 }, e: { r: 4, c: 2 } }, // แหล่งเงิน
    { s: { r: 3, c: 3 }, e: { r: 4, c: 3 } }, // ผลผลิต
    { s: { r: 3, c: 4 }, e: { r: 4, c: 4 } }, // ลักษณะโครงการ
    { s: { r: 3, c: 5 }, e: { r: 4, c: 5 } }, // งบประมาณ

    // "จำนวนผู้เข้าร่วมโครงการ (คน)" ครอบ 4 ช่อง
    { s: { r: 3, c: 6 }, e: { r: 3, c: 9 } },
    ...
    // "ความสอดคล้องกับแผนกลยุทธ์การพัฒนา" ครอบ 5 ช่อง
    { s: { r: 3, c: 12 }, e: { r: 3, c: 16 } },

    // "ตัวชี้วัดความสำเร็จของโครงการ" ครอบ 2 ช่อง
    ...
    // 🔹 รวมทั้งหมด = ส้ม
      ...
      for (let C = range.s.c; C <= range.e.c; ++C) {
        ...
        if (cell && cell.v && typeof cell.v === "string") {
          ...
          if (text.includes("รวมทั้งหมด")) {
            bgColor = "F4A460"; // ส้มอ่อน
          } else if (text.includes("คณะ") || text.includes("FACULTY")) {
            bgColor = "8DF5F7"; // ฟ้าอ่อน
          }
        }
      }
    }
  ...
  for (let R = range.s.r; R <= range.e.r; ++R) {
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      if (cell && typeof cell.v === "number") {
        if (C === 0) {
          cell.z = "0"; // ลำดับ ไม่มีทศนิยม
        } else if (C >= 3 && (C % 2 === 1)) {
          cell.z = "#,##0.00"; // งบประมาณ มีทศนิยม 2 ตำแหน่ง
        } else {
          cell.z = "#,##0"; // ค่าอื่นๆ ไม่มีทศนิยม
        }
      }
    }
  }
