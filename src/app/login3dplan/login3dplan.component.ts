...
@Component({
  selector: 'app-login3dplan',
  templateUrl: './login3dplan.component.html',
  styleUrls: ['./login3dplan.component.scss']
})
export class Login3dplanComponent implements OnInit {
  form:any;
  isLoggedIn = false;
  roles: string[] = [];
  loading = false;
  ...
  ngOnInit(): void {
    ...
    //---------------
     const redirect =
    this.route.snapshot.queryParamMap.get('redirect') || 'hr/';

    if (this.tokenStorage.getToken()) {
      this.isLoggedIn = true;
      const user = this.tokenStorage.getUser();
      //this.router.navigate(['acc3d/investment/']);
